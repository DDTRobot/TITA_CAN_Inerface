...
bool tita_robot::data_time_out() const
{
    return can_receiver_->is_timeout();
}
...
std::vector<uint16_t> tita_robot::get_joint_status() const
{
...
// std::vector<double> tita_robot::get_jump_status() const
// {
...
bool tita_robot::set_joint_torque(const std::vector<double> &t)
{
...
bool tita_robot::set_robot_stop()
{
  ...
  return_ok &= can_sender_->send_command_can_rpc_request(rpc_request);
  std::this_thread::sleep_for(std::chrono::microseconds(200));
  rpc_request.value = 0x00U;
  rpc_request.key = can_device::SET_JUMP;
  return_ok &= can_sender_->send_command_can_rpc_request(rpc_request);
  return true;
}
bool tita_robot::set_robot_down(bool down )
{
    can_device::api_rpc_response_t rpc_request;
    rpc_request.key = can_device::SET_STAND_MODE;
    rpc_request.value = !down ? 0x01U : 0x04U;
    return can_sender_->send_command_can_rpc_request(rpc_request);
